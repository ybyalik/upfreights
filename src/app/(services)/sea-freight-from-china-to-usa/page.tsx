...
const majorPorts = [
  { name: 'Los Angeles', transit: '15-18 days', volume: 'Highest volume port' },
  { name: 'Long Beach', transit: '15-18 days', volume: 'Major West Coast hub' },
  { name: 'New York', transit: '22-28 days', volume: 'East Coast gateway' },
  { name: 'Savannah', transit: '25-30 days', volume: 'Southeast distribution' },
  { name: 'Seattle', transit: '12-15 days', volume: 'Pacific Northwest' },
];

export default function SeaFreightChinaToUSAPage() {
  // Get sea routes for United States only
  const seaRoutes = generateSeaRoutes().filter(r =>
    r.destinationCountry.toLowerCase() === 'united states' ||
    r.destinationCountry.toLowerCase().includes('united')
  );
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-lg text-white/80 mb-8">
                Cost-effective ocean shipping with FCL and LCL options from major Chinese ports to all US destinations.
                Transit times from 12-30 days depending on port selection.
              </p>
      ...
      <section className="py-12 bg-secondary/30">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <div className="text-center">
              <div className="text-3xl md:text-4xl font-bold text-ocean mb-1">12-30</div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-6">
                Why Choose Sea Freight to the USA?
              </h2>
              <p className="text-muted-foreground mb-8">
                Sea freight is the most economical way to ship large volumes from China to the United States.
                Whether you need full container loads or shared container space, we offer flexible solutions
                with reliable transit times and comprehensive tracking.
              </p>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground max-w-2xl mx-auto">
              We ship to all major US ports with optimized routes for fastest transit and best rates.
            </p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {majorPorts.map((port) => (
              <a key={port.name} href={`#routes-${port.name.toLowerCase().replace(/\s+/g, '-')}`} className="block">
                <Card className="border-border/50 hover:border-ocean/50 hover:shadow-md transition-all cursor-pointer">
                  <CardContent className="p-6">
        ...
        <div className="container mx-auto px-4">
          <div className="mb-8">
            <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-2">
              All Sea Freight Routes to USA
            </h2>
            <p className="text-muted-foreground">
              Browse all {seaRoutes.length} sea freight routes from China to the United States.
            </p>
          </div>

          <div className="space-y-8">
            {Object.entries(routesByDestination).map(([destination, routes]) => (
              <div key={destination} id={`routes-${destination.toLowerCase().replace(/\s+/g, '-')}`} className="scroll-mt-24">
                <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
