...
const SLUG = 'shipping-china-to-germany';
const country = getCountryBySlug(SLUG);

const title = country?.content?.metaTitle ?? `Shipping from China to ${country?.name ?? 'Germany'}`;
...
export default function ShippingToGermanyPage() {
  ...
  const seaRoutes = generateSeaRoutes().filter(
    (r) => r.destinationCountry.toLowerCase() === 'germany'
  );
