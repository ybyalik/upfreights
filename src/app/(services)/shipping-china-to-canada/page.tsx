...
const SLUG = 'shipping-china-to-canada';
const country = getCountryBySlug(SLUG);

const title = country?.content?.metaTitle ?? `Shipping from China to ${country?.name ?? 'Canada'}`;
...
export default function ShippingToCanadaPage() {
  ...
  const seaRoutes = generateSeaRoutes().filter(
    (r) => r.destinationCountry.toLowerCase() === 'canada'
  );
