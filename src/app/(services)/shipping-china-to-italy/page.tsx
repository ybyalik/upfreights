...
const SLUG = 'shipping-china-to-italy';
const country = getCountryBySlug(SLUG);

const title = country?.content?.metaTitle ?? `Shipping from China to ${country?.name ?? 'Italy'}`;
...
export default function ShippingToItalyPage() {
  ...
  const seaRoutes = generateSeaRoutes().filter(
    (r) => r.destinationCountry.toLowerCase() === 'italy'
  );
